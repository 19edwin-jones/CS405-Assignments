...
// DONE: Create a test to verify that max size is greater than or equal to size for 0, 1, 5, 10 entries ✔︎
TEST_F(CollectionTest, MaxSizeIsGreaterThanOrEqualToSize) {
  ...
  add_entries(1);  ASSERT_EQ(collection->size(), 1);
  ASSERT_GE(collection->max_size(), collection->size());

  // Add 5 entries and check
  add_entries(4);  ASSERT_EQ(collection->size(), 5);
  ASSERT_GE(collection->max_size(), collection->size());

  // Add 10 entries and check
  add_entries(5);  ASSERT_EQ(collection->size(), 10);
  ASSERT_GE(collection->max_size(), collection->size());
}
...
TEST_F(CollectionTest, AtThrowsOutOfRange) {
  ...
  // Attempt to access an element at index 0, which should throw std::out_of_range
  ASSERT_THROW({
    collection->at(0);
  }, std::out_of_range);
}
...
// Positive test to verify that swapping contents of two collections works correctly
...
// Negative test to verify that accessing an out-of-bounds index throws std::out_of_range
